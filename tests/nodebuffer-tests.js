// base64: ignore whitespace
var text = '\n   YW9ldQ==  '
var buf = new Buffer(text, 'base64')
assert(buf.toString() === 'aoeu')

// base64: strings without padding
assert((new Buffer('YW9ldQ', 'base64').toString()) === 'aoeu')

// base64: newline in utf8 -- should not be an issue
assert(new Buffer('LS0tCnRpdGxlOiBUaHJlZSBkYXNoZXMgbWFya3MgdGhlIHNwb3QKdGFnczoK', 'base64').toString('utf8') === '---\ntitle: Three dashes marks the spot\ntags:\n')

// base64: newline in base64 -- should get stripped
assert(new Buffer('LS0tCnRpdGxlOiBUaHJlZSBkYXNoZXMgbWFya3MgdGhlIHNwb3QKdGFnczoK\nICAtIHlhbWwKICAtIGZyb250LW1hdHRlcgogIC0gZGFzaGVzCmV4cGFuZWQt', 'base64').toString('utf8') === '---\ntitle: Three dashes marks the spot\ntags:\n  - yaml\n  - front-matter\n  - dashes\nexpaned-')

// base64: tab characters in base64 - should get stripped
assert(new Buffer('LS0tCnRpdGxlOiBUaHJlZSBkYXNoZXMgbWFya3MgdGhlIHNwb3QKdGFnczoK\t\t\t\tICAtIHlhbWwKICAtIGZyb250LW1hdHRlcgogIC0gZGFzaGVzCmV4cGFuZWQt', 'base64').toString('utf8') === '---\ntitle: Three dashes marks the spot\ntags:\n  - yaml\n  - front-matter\n  - dashes\nexpaned-')

// base64: invalid non-alphanumeric characters -- should be stripped
assert(new Buffer('!"#$%&\'()*,.:;<=>?@[\\]^`{|}~', 'base64').toString('utf8') === '')

// instanceof Buffer
var buf = new Buffer([1, 2])
assert(buf instanceof Buffer)

// convert to Uint8Array in modern browsers
if (Buffer.TYPED_ARRAY_SUPPORT) {
    var buf = new Buffer([1, 2])
    var uint8array = new Uint8Array(buf.buffer)
    assert(uint8array instanceof Uint8Array)
    assert(uint8array[0] === 1)
    assert(uint8array[1] === 2)
}

// indexes from a string
var buf = new Buffer('abc')
assert(buf[0] === 97)
assert(buf[1] === 98)
assert(buf[2] === 99)

// indexes from an array
var buf = new Buffer([ 97, 98, 99 ])
assert(buf[0] === 97)
assert(buf[1] === 98)
assert(buf[2] === 99)

// setting index value should modify buffer contents
var buf = new Buffer([ 97, 98, 99 ])
assert(buf[2] === 99)
assert(buf.toString() === 'abc')
buf[2] += 10
assert(buf[2] === 109)
assert(buf.toString() === 'abm')

// storing negative number should cast to unsigned
var buf = new Buffer(1)
if (Buffer.TYPED_ARRAY_SUPPORT) {
    // This does not work with the object implementation -- nothing we can do!
    buf[0] = -3
    assert(buf[0] === 253)
}
buf = new Buffer(1)
buf.writeInt8(-3, 0)
assert(buf[0] === 253)

// test that memory is copied from array-like
if (Buffer.TYPED_ARRAY_SUPPORT) {
    var u = new Uint8Array(4)
    var b = new Buffer(u)
    b[0] = 1
    b[1] = 2
    b[2] = 3
    b[3] = 4
    
    assert(u[0] === 0)
    assert(u[1] === 0)
    assert(u[2] === 0)
    assert(u[3] === 0)
}

// buffer.compare
var b = new Buffer(1).fill('a')
var c = new Buffer(1).fill('c')
var d = new Buffer(2).fill('aa')
assert(b.compare(c) === -1)
assert(c.compare(d) === 1)
assert(d.compare(b) === 1)
assert(b.compare(d) === -1)

// static method
assert(Buffer.compare(b, c) === -1)
assert(Buffer.compare(c, d) === 1)
assert(Buffer.compare(d, b) === 1)
assert(Buffer.compare(b, d) === -1)

// buffer.equals
var b = new Buffer(5).fill('abcdf')
var c = new Buffer(5).fill('abcdf')
var d = new Buffer(5).fill('abcde')
var e = new Buffer(6).fill('abcdef')
assert(b.equals(c))
assert(!c.equals(d))
assert(!d.equals(e))

// new buffer from array
assert(new Buffer([1, 2, 3]).toString() === '\u0001\u0002\u0003')

// new buffer from array w/ negatives
assert(new Buffer([-1, -2, -3]).toString('hex') === 'fffefd')

// new buffer from array with mixed signed input
assert(new Buffer([-255, 255, -128, 128, 512, -512, 511, -511]).toString('hex') === '01ff80800000ff01')

// new buffer from string
assert(new Buffer('hey', 'utf8').toString() ===	'hey')

// new buffer from buffer
var b1 = new Buffer('asdf')
var b2 = new Buffer(b1)
assert(b1.toString('hex') === b2.toString('hex'))

// new buffer from ArrayBuffer
if (typeof ArrayBuffer !== 'undefined') {
    var arraybuffer = new Uint8Array([0, 1, 2, 3]).buffer
    var b = new Buffer(arraybuffer)
    assert(b.length === 4)
    assert(b[0] === 0)
    assert(b[1] === 1)
    assert(b[2] === 2)
    assert(b[3] === 3)
    assert(b[4] === undefined)
}

// new buffer from ArrayBuffer, shares memory
if (Buffer.TYPED_ARRAY_SUPPORT) {
    var u = new Uint8Array([0, 1, 2, 3])
    var arraybuffer = u.buffer
    var b = new Buffer(arraybuffer)
    assert(b.length === 4)
    assert(b[0] === 0)
    assert(b[1] === 1)
    assert(b[2] === 2)
    assert(b[3] === 3)
    assert(b[4] === undefined)
    
    // changing the Uint8Array (and thus the ArrayBuffer), changes the Buffer
    u[0] = 10
    assert(b[0] === 10)
    u[1] = 11
    assert(b[1] === 11)
    u[2] = 12
    assert(b[2] === 12)
    u[3] = 13
    assert(b[3] === 13)
}

// new buffer from Uint8Array
if (typeof Uint8Array !== 'undefined') {
    var b1 = new Uint8Array([0, 1, 2, 3])
    var b2 = new Buffer(b1)
    assert(b1.length === b2.length)
    assert(b1[0] === 0)
    assert(b1[1] === 1)
    assert(b1[2] === 2)
    assert(b1[3] === 3)
    assert(b1[4] === undefined)
}

// new buffer from Uint16Array
if (typeof Uint16Array !== 'undefined') {
    var b1 = new Uint16Array([0, 1, 2, 3])
    var b2 = new Buffer(b1)
    assert(b1.length === b2.length)
    assert(b1[0] === 0)
    assert(b1[1] === 1)
    assert(b1[2] === 2)
    assert(b1[3] === 3)
    assert(b1[4] === undefined)
}

// new buffer from Uint32Array
if (typeof Uint32Array !== 'undefined') {
    var b1 = new Uint32Array([0, 1, 2, 3])
    var b2 = new Buffer(b1)
    assert(b1.length === b2.length)
    assert(b1[0] === 0)
    assert(b1[1] === 1)
    assert(b1[2] === 2)
    assert(b1[3] === 3)
    assert(b1[4] === undefined)
}

// new buffer from Int16Array
if (typeof Int16Array !== 'undefined') {
    var b1 = new Int16Array([0, 1, 2, 3])
    var b2 = new Buffer(b1)
    assert(b1.length === b2.length)
    assert(b1[0] === 0)
    assert(b1[1] === 1)
    assert(b1[2] === 2)
    assert(b1[3] === 3)
    assert(b1[4] === undefined)
}

// new buffer from Int32Array
if (typeof Int32Array !== 'undefined') {
    var b1 = new Int32Array([0, 1, 2, 3])
    var b2 = new Buffer(b1)
    assert(b1.length === b2.length)
    assert(b1[0] === 0)
    assert(b1[1] === 1)
    assert(b1[2] === 2)
    assert(b1[3] === 3)
    assert(b1[4] === undefined)
}

// new buffer from Float32Array
if (typeof Float32Array !== 'undefined') {
    var b1 = new Float32Array([0, 1, 2, 3])
    var b2 = new Buffer(b1)
    assert(b1.length === b2.length)
    assert(b1[0] === 0)
    assert(b1[1] === 1)
    assert(b1[2] === 2)
    assert(b1[3] === 3)
    assert(b1[4] === undefined)
}

// new buffer from Float64Array
if (typeof Float64Array !== 'undefined') {
    var b1 = new Float64Array([0, 1, 2, 3])
    var b2 = new Buffer(b1)
    assert(b1.length === b2.length)
    assert(b1[0] === 0)
    assert(b1[1] === 1)
    assert(b1[2] === 2)
    assert(b1[3] === 3)
    assert(b1[4] === undefined)
}

// new buffer from buffer.toJSON() output
var buf = new Buffer('test')
var json = JSON.stringify(buf)
var obj = JSON.parse(json)
var copy = new Buffer(obj)
assert(buf.equals(copy))


// ######################################## MANUAL TESTS ##############################################
// buffer.compare argument validation
var b = new Buffer(1)
//Buffer.compare(b, 'abc')  // Uncomment this and it should throw an exception.
var b = new Buffer(1)
//Buffer.compare('abc', b)  // Uncomment this and it should throw an exception.
var b = new Buffer(1)
//b.compare('abc')     // Uncomment this and it should throw an exception

// buffer.equals argument validation
var b = new Buffer(1)
//b.equals('abc')        // Uncomment this and it should throw an exception






// detect utf16 surrogate pairs
var text = '\uD83D\uDE38' + '\uD83D\uDCAD' + '\uD83D\uDC4D'
var buf = new Buffer(text)
assert(text === buf.toString())

// detect utf16 surrogate pairs over U+20000 until U+10FFFF
var text = '\uD842\uDFB7' + '\uD93D\uDCAD' + '\uDBFF\uDFFF'
var buf = new Buffer(text)
assert(text === buf.toString())

// replace orphaned utf16 surrogate lead code point
var text = '\uD83D\uDE38' + '\uD83D' + '\uD83D\uDC4D'
var buf = new Buffer(text)
assert(deepEqual(buf, new Buffer([ 0xf0, 0x9f, 0x98, 0xb8, 0xef, 0xbf, 0xbd, 0xf0, 0x9f, 0x91, 0x8d ])))

// replace orphaned utf16 surrogate trail code point
var text = '\uD83D\uDE38' + '\uDCAD' + '\uD83D\uDC4D'
var buf = new Buffer(text)
assert(deepEqual(buf, new Buffer([ 0xf0, 0x9f, 0x98, 0xb8, 0xef, 0xbf, 0xbd, 0xf0, 0x9f, 0x91, 0x8d ])))

// do not write partial utf16 code units
var f = new Buffer([0, 0, 0, 0, 0])
assert(f.length === 5)
var size = f.write('あいうえお', 'utf16le')
assert(size === 4)
assert(deepEqual(f, new Buffer([0x42, 0x30, 0x44, 0x30, 0x00])))

// handle partial utf16 code points when encoding to utf8 the way node does
var text = '\uD83D\uDE38' + '\uD83D\uDC4D'

var buf = new Buffer(8)
buf.fill(0)
buf.write(text)
assert(deepEqual(buf, new Buffer([ 0xf0, 0x9f, 0x98, 0xb8, 0xf0, 0x9f, 0x91, 0x8d ])))

buf = new Buffer(7)
buf.fill(0)
buf.write(text)
assert(deepEqual(buf, new Buffer([ 0xf0, 0x9f, 0x98, 0xb8, 0x00, 0x00, 0x00 ])))

buf = new Buffer(6)
buf.fill(0)
buf.write(text)
assert(deepEqual(buf, new Buffer([ 0xf0, 0x9f, 0x98, 0xb8, 0x00, 0x00 ])))

buf = new Buffer(5)
buf.fill(0)
buf.write(text)
assert(deepEqual(buf, new Buffer([ 0xf0, 0x9f, 0x98, 0xb8, 0x00 ])))

buf = new Buffer(4)
buf.fill(0)
buf.write(text)
assert(deepEqual(buf, new Buffer([ 0xf0, 0x9f, 0x98, 0xb8 ])))

buf = new Buffer(3)
buf.fill(0)
buf.write(text)
assert(deepEqual(buf, new Buffer([ 0x00, 0x00, 0x00 ])))

buf = new Buffer(2)
buf.fill(0)
buf.write(text)
assert(deepEqual(buf, new Buffer([ 0x00, 0x00 ])))

buf = new Buffer(1)
buf.fill(0)
buf.write(text)
assert(deepEqual(buf, new Buffer([ 0x00 ])))

// handle invalid utf16 code points when encoding to utf8 the way node does
var text = 'a' + '\uDE38\uD83D' + 'b'

var buf = new Buffer(8)
buf.fill(0)
buf.write(text)
assert(deepEqual(buf, new Buffer([ 0x61, 0xef, 0xbf, 0xbd, 0xef, 0xbf, 0xbd, 0x62 ])))

buf = new Buffer(7)
buf.fill(0)
buf.write(text)
assert(deepEqual(buf, new Buffer([ 0x61, 0xef, 0xbf, 0xbd, 0xef, 0xbf, 0xbd ])))

buf = new Buffer(6)
buf.fill(0)
buf.write(text)
assert(deepEqual(buf, new Buffer([ 0x61, 0xef, 0xbf, 0xbd, 0x00, 0x00 ])))

buf = new Buffer(5)
buf.fill(0)
buf.write(text)
assert(deepEqual(buf, new Buffer([ 0x61, 0xef, 0xbf, 0xbd, 0x00 ])))

buf = new Buffer(4)
buf.fill(0)
buf.write(text)
assert(deepEqual(buf, new Buffer([ 0x61, 0xef, 0xbf, 0xbd ])))

buf = new Buffer(3)
buf.fill(0)
buf.write(text)
assert(deepEqual(buf, new Buffer([ 0x61, 0x00, 0x00 ])))

buf = new Buffer(2)
buf.fill(0)
buf.write(text)
assert(deepEqual(buf, new Buffer([ 0x61, 0x00 ])))

buf = new Buffer(1)
buf.fill(0)
buf.write(text)
assert(deepEqual(buf, new Buffer([ 0x61 ])))



// ############################## METHODS TESTS ##################################
// buffer.toJSON
var data = [1, 2, 3, 4]
assert(deepEqual(new Buffer(data).toJSON(), { type: 'Buffer', data: [ 1, 2, 3, 4 ] }))

// buffer.copy
// copied from nodejs.org example
var buf1 = new Buffer(26)
var buf2 = new Buffer(26)
for (var i = 0; i < 26; i++) {
    buf1[i] = i + 97 // 97 is ASCII a
    buf2[i] = 33 // ASCII !
}
buf1.copy(buf2, 8, 16, 20)
assert(buf2.toString('ascii', 0, 25) === '!!!!!!!!qrst!!!!!!!!!!!!!')

// test offset returns are correct
var b = new Buffer(16)
assert(4 === b.writeUInt32LE(0, 0))
assert(6 === b.writeUInt16LE(0, 4))
assert(7 === b.writeUInt8(0, 6))
assert(8 === b.writeInt8(0, 7))
assert(16 === b.writeDoubleLE(0, 8))

// concat() a varying number of buffers
var zero = []
var one = [ new Buffer('asdf') ]
var long = []
for (var i = 0; i < 10; i++) {
    long.push(new Buffer('asdf'))
}
var flatZero = Buffer.concat(zero)
var flatOne = Buffer.concat(one)
var flatLong = Buffer.concat(long)
var flatLongLen = Buffer.concat(long, 40)
assert(flatZero.length === 0)
assert(flatOne.toString() === 'asdf')
assert(deepEqual(flatOne, one[0]))
assert(flatLong.toString() === (new Array(10 + 1).join('asdf')))
assert(flatLongLen.toString() === (new Array(10 + 1).join('asdf')))

// fill
var b = new Buffer(10)
b.fill(2)
assert(b.toString('hex') === '02020202020202020202')

// fill (string)
var b = new Buffer(10)
b.fill('abc')
assert(b.toString() === 'abcabcabca')
b.fill('է')
assert(b.toString() === 'էէէէէ')

// copy() empty buffer with sourceEnd=0
var source = new Buffer([42])
var destination = new Buffer([43])
source.copy(destination, 0, 0, 0)
assert(destination.readUInt8(0) === 43)

// copy() after slice()
var source = new Buffer(200)
var dest = new Buffer(200)
var expected = new Buffer(200)
for (var i = 0; i < 200; i++) {
    source[i] = i
    dest[i] = 0
}
source.slice(2).copy(dest)
source.copy(expected, 0, 2)
assert(deepEqual(dest, expected))

// copy() ascending
var b = new Buffer('abcdefghij')
b.copy(b, 0, 3, 10)
assert(b.toString() === 'defghijhij')

// copy() descending
var b = new Buffer('abcdefghij')
b.copy(b, 3, 0, 7)
assert(b.toString() === 'abcabcdefg')

// buffer.slice sets indexes
assert((new Buffer('hallo')).slice(0, 5).toString() === 'hallo')

// buffer.slice out of range
assert((new Buffer('hallo')).slice(0, 10).toString() === 'hallo')
assert((new Buffer('hallo')).slice(10, 2).toString() === '')


// ##################################### SLICE TESTS ########################################
// modifying buffer created by .slice() modifies original memory{
var buf1 = new Buffer(26)
for (var i = 0; i < 26; i++) {
    buf1[i] = i + 97 // 97 is ASCII a
}
var buf2 = buf1.slice(0, 3)
assert(buf2.toString('ascii', 0, buf2.length) === 'abc')
buf2[0] = '!'.charCodeAt(0)
assert(buf1.toString('ascii', 0, buf2.length) === '!bc')

// modifying parent buffer modifies .slice() buffer\'s memory
var buf1 = new Buffer(26)
for (var i = 0; i < 26; i++) {
    buf1[i] = i + 97 // 97 is ASCII a
}
var buf2 = buf1.slice(0, 3)
assert(buf2.toString('ascii', 0, buf2.length) === 'abc')
buf1[0] = '!'.charCodeAt(0)
assert(buf2.toString('ascii', 0, buf2.length) === '!bc')


// ####################################### STATIC TESTS #######################################
assert(Buffer.isEncoding('HEX') === true)
assert(Buffer.isEncoding('hex') === true)
assert(Buffer.isEncoding('bad') === false)
assert(Buffer.isBuffer(new Buffer('hey', 'utf8')) === true)
assert(Buffer.isBuffer(new Buffer([1, 2, 3], 'utf8')) === true)
assert(Buffer.isBuffer('hey') === false)


// ####################################### TO-STRING TESTS #######################################
// utf8 buffer to base64
assert(new Buffer('Ձאab', 'utf8').toString('base64') === '1YHXkGFi')

// utf8 buffer to hex
assert(new Buffer('Ձאab', 'utf8').toString('hex') === 'd581d7906162')

// utf8 to utf8
assert(new Buffer('öäüõÖÄÜÕ', 'utf8').toString('utf8') === 'öäüõÖÄÜÕ')

// utf16le to utf16
assert(new Buffer(new Buffer('abcd', 'utf8').toString('utf16le'), 'utf16le').toString('utf8') === 'abcd')

// utf16le to hex
assert(new Buffer('abcd', 'utf16le').toString('hex') === '6100620063006400')

// ascii buffer to base64
assert(new Buffer('123456!@#$%^', 'ascii').toString('base64') === 'MTIzNDU2IUAjJCVe')

// ascii buffer to hex
assert(new Buffer('123456!@#$%^', 'ascii').toString('hex') === '31323334353621402324255e')

// base64 buffer to utf8
assert(new Buffer('1YHXkGFi', 'base64').toString('utf8') === 'Ձאab')

// hex buffer to utf8
assert(new Buffer('d581d7906162', 'hex').toString('utf8') === 'Ձאab')

// base64 buffer to ascii
assert(new Buffer('MTIzNDU2IUAjJCVe', 'base64').toString('ascii') === '123456!@#$%^')

// hex buffer to ascii
assert(new Buffer('31323334353621402324255e', 'hex').toString('ascii') === '123456!@#$%^')

// base64 buffer to binary
assert(new Buffer('MTIzNDU2IUAjJCVe', 'base64').toString('binary') === '123456!@#$%^')

// hex buffer to binary
assert(new Buffer('31323334353621402324255e', 'hex').toString('binary') === '123456!@#$%^')

// utf8 to binary
/* jshint -W100 */
assert(new Buffer('öäüõÖÄÜÕ', 'utf8').toString('binary') === 'Ã¶Ã¤Ã¼ÃµÃÃÃÃ')

// utf8 replacement chars (1 byte sequence)
assert(new Buffer([ 0x80 ]).toString() === '\uFFFD')
assert(new Buffer([ 0x7F ]).toString() === '\u007F')

// utf8 replacement chars (2 byte sequences)
assert(new Buffer([ 0xC7 ]).toString() === '\uFFFD')
assert(new Buffer([ 0xC7, 0xB1 ]).toString() === '\u01F1')
assert(new Buffer([ 0xC0, 0xB1 ]).toString() === '\uFFFD\uFFFD')
assert(new Buffer([ 0xC1, 0xB1 ]).toString() === '\uFFFD\uFFFD')

// utf8 replacement chars (3 byte sequences)
assert(new Buffer([ 0xE0 ]).toString() === '\uFFFD')
assert(new Buffer([ 0xE0, 0xAC ]).toString() === '\uFFFD\uFFFD')
assert(new Buffer([ 0xE0, 0xAC, 0xB9 ]).toString() === '\u0B39')

// utf8 replacement chars (4 byte sequences)
assert(new Buffer([ 0xF4 ]).toString() === '\uFFFD')
assert(new Buffer([ 0xF4, 0x8F ]).toString() === '\uFFFD\uFFFD')
assert(new Buffer([ 0xF4, 0x8F, 0x80 ]).toString() === '\uFFFD\uFFFD\uFFFD')
assert(new Buffer([ 0xF4, 0x8F, 0x80, 0x84 ]).toString() === '\uDBFC\uDC04')
assert(new Buffer([ 0xFF ]).toString() === '\uFFFD')
assert(new Buffer([ 0xFF, 0x8F, 0x80, 0x84 ]).toString() === '\uFFFD\uFFFD\uFFFD\uFFFD')

// utf8 replacement chars on 256 random bytes
assert(new Buffer([ 152, 130, 206, 23, 243, 238, 197, 44, 27, 86, 208, 36, 163, 184, 164, 21, 94, 242, 178, 46, 25, 26, 253, 178, 72, 147, 207, 112, 236, 68, 179, 190, 29, 83, 239, 147, 125, 55, 143, 19, 157, 68, 157, 58, 212, 224, 150, 39, 128, 24, 94, 225, 120, 121, 75, 192, 112, 19, 184, 142, 203, 36, 43, 85, 26, 147, 227, 139, 242, 186, 57, 78, 11, 102, 136, 117, 180, 210, 241, 92, 3, 215, 54, 167, 249, 1, 44, 225, 146, 86, 2, 42, 68, 21, 47, 238, 204, 153, 216, 252, 183, 66, 222, 255, 15, 202, 16, 51, 134, 1, 17, 19, 209, 76, 238, 38, 76, 19, 7, 103, 249, 5, 107, 137, 64, 62, 170, 57, 16, 85, 179, 193, 97, 86, 166, 196, 36, 148, 138, 193, 210, 69, 187, 38, 242, 97, 195, 219, 252, 244, 38, 1, 197, 18, 31, 246, 53, 47, 134, 52, 105, 72, 43, 239, 128, 203, 73, 93, 199, 75, 222, 220, 166, 34, 63, 236, 11, 212, 76, 243, 171, 110, 78, 39, 205, 204, 6, 177, 233, 212, 243, 0, 33, 41, 122, 118, 92, 252, 0, 157, 108, 120, 70, 137, 100, 223, 243, 171, 232, 66, 126, 111, 142, 33, 3, 39, 117, 27, 107, 54, 1, 217, 227, 132, 13, 166, 3, 73, 53, 127, 225, 236, 134, 219, 98, 214, 125, 148, 24, 64, 142, 111, 231, 194, 42, 150, 185, 10, 182, 163, 244, 19, 4, 59, 135, 16 ]).toString() ===
	'\uFFFD\uFFFD\uFFFD\u0017\uFFFD\uFFFD\uFFFD\u002C\u001B\u0056\uFFFD\u0024\uFFFD\uFFFD\uFFFD\u0015\u005E\uFFFD\uFFFD\u002E\u0019\u001A\uFFFD\uFFFD\u0048\uFFFD\uFFFD\u0070\uFFFD\u0044\uFFFD\uFFFD\u001D\u0053\uFFFD\uFFFD\u007D\u0037\uFFFD\u0013\uFFFD\u0044\uFFFD\u003A\uFFFD\uFFFD\uFFFD\u0027\uFFFD\u0018\u005E\uFFFD\u0078\u0079\u004B\uFFFD\u0070\u0013\uFFFD\uFFFD\uFFFD\u0024\u002B\u0055\u001A\uFFFD\uFFFD\uFFFD\uFFFD\uFFFD\u0039\u004E\u000B\u0066\uFFFD\u0075\uFFFD\uFFFD\uFFFD\u005C\u0003\uFFFD\u0036\uFFFD\uFFFD\u0001\u002C\uFFFD\uFFFD\u0056\u0002\u002A\u0044\u0015\u002F\uFFFD\u0319\uFFFD\uFFFD\uFFFD\u0042\uFFFD\uFFFD\u000F\uFFFD\u0010\u0033\uFFFD\u0001\u0011\u0013\uFFFD\u004C\uFFFD\u0026\u004C\u0013\u0007\u0067\uFFFD\u0005\u006B\uFFFD\u0040\u003E\uFFFD\u0039\u0010\u0055\uFFFD\uFFFD\u0061\u0056\uFFFD\uFFFD\u0024\uFFFD\uFFFD\uFFFD\uFFFD\u0045\uFFFD\u0026\uFFFD\u0061\uFFFD\uFFFD\uFFFD\uFFFD\u0026\u0001\uFFFD\u0012\u001F\uFFFD\u0035\u002F\uFFFD\u0034\u0069\u0048\u002B\uFFFD\uFFFD\uFFFD\u0049\u005D\uFFFD\u004B\uFFFD\u0726\u0022\u003F\uFFFD\u000B\uFFFD\u004C\uFFFD\uFFFD\u006E\u004E\u0027\uFFFD\uFFFD\u0006\uFFFD\uFFFD\uFFFD\uFFFD\u0000\u0021\u0029\u007A\u0076\u005C\uFFFD\u0000\uFFFD\u006C\u0078\u0046\uFFFD\u0064\uFFFD\uFFFD\uFFFD\uFFFD\u0042\u007E\u006F\uFFFD\u0021\u0003\u0027\u0075\u001B\u006B\u0036\u0001\uFFFD\uFFFD\uFFFD\u000D\uFFFD\u0003\u0049\u0035\u007F\uFFFD\uFFFD\uFFFD\uFFFD\u0062\uFFFD\u007D\uFFFD\u0018\u0040\uFFFD\u006F\uFFFD\uFFFD\u002A\uFFFD\uFFFD\u000A\uFFFD\uFFFD\uFFFD\u0013\u0004\u003B\uFFFD\u0010')

// utf8 replacement chars for anything in the surrogate pair range
assert(new Buffer([ 0xED, 0x9F, 0xBF ]).toString() === '\uD7FF')
assert(new Buffer([ 0xED, 0xA0, 0x80 ]).toString() === '\uFFFD\uFFFD\uFFFD')
assert(new Buffer([ 0xED, 0xBE, 0x8B ]).toString() === '\uFFFD\uFFFD\uFFFD')
assert(new Buffer([ 0xED, 0xBF, 0xBF ]).toString() === '\uFFFD\uFFFD\uFFFD')
assert(new Buffer([ 0xEE, 0x80, 0x80 ]).toString() === '\uE000')

// utf8 don\'t replace the replacement char
assert(new Buffer('\uFFFD').toString() === '\uFFFD')



// ####################################### WRITE TESTS ###########################################
// buffer.write string should get parsed as number
var b = new Buffer(64)
b.writeUInt16LE('1003', 0)
assert(b.readUInt16LE(0) === 1003)

// buffer.writeUInt8 a fractional number will get Math.floored
// Some extra work is necessary to make this test pass with the Object implementation
var b = new Buffer(1)
b.writeInt8(5.5, 0)
assert(b[0] === 5)

// writeUint8 with a negative number throws
var buf = new Buffer(1)
//buf.writeUInt8(-3, 0)    // Uncomment this and it should throw an error

// hex of write{Uint,Int}{8,16,32}{LE,BE}
var hex = [
    '03', '0300', '0003', '03000000', '00000003',
    'fd', 'fdff', 'fffd', 'fdffffff', 'fffffffd'
]
var reads = [ 3, 3, 3, 3, 3, -3, -3, -3, -3, -3 ]
var xs = ['UInt', 'Int']
var ys = [8, 16, 32]
for (var i = 0; i < xs.length; i++) {
    var x = xs[i]
    for (var j = 0; j < ys.length; j++) {
	var y = ys[j]
	var endianesses = (y === 8) ? [''] : ['LE', 'BE']
	for (var k = 0; k < endianesses.length; k++) {
	    var z = endianesses[k]
	    
	    var v1 = new Buffer(y / 8)
	    var writefn = 'write' + x + y + z
	    var val = (x === 'Int') ? -3 : 3
	    v1[writefn](val, 0)
	    assert(v1.toString('hex') === hex.shift())
	    var readfn = 'read' + x + y + z
	    assert(v1[readfn](0) === reads.shift())
	}
    }
}

// hex of write{Uint,Int}{8,16,32}{LE,BE} with overflow
var hex = [
    '', '03', '00', '030000', '000000',
    '', 'fd', 'ff', 'fdffff', 'ffffff'
]
var reads = [
    undefined, 3, 0, NaN, 0,
    undefined, 253, -256, 16777213, -256
]
var xs = ['UInt', 'Int']
var ys = [8, 16, 32]
for (var i = 0; i < xs.length; i++) {
    var x = xs[i]
    for (var j = 0; j < ys.length; j++) {
	var y = ys[j]
	var endianesses = (y === 8) ? [''] : ['LE', 'BE']
	for (var k = 0; k < endianesses.length; k++) {
	    var z = endianesses[k]
	    
	    var v1 = new Buffer(y / 8 - 1)
	    var next = new Buffer(4)
	    next.writeUInt32BE(0, 0)
	    var writefn = 'write' + x + y + z
	    var val = (x === 'Int') ? -3 : 3
	    v1[writefn](val, 0, true)
	    assert(v1.toString('hex') === hex.shift())
	    // check that nothing leaked to next buffer.
	    assert(next.readUInt32BE(0) === 0)
	    // check that no bytes are read from next buffer.
	    next.writeInt32BE(~0, 0)
	    var readfn = 'read' + x + y + z
	    var r = reads.shift()
	    if (isNaN(r)) assert(true)
	    else assert(v1[readfn](0, true) === r)
	}
    }
}

// large values do not improperly roll over (ref #80)
var nums = [-25589992, -633756690, -898146932]
var out = new Buffer(12)
out.fill(0)
out.writeInt32BE(nums[0], 0)
var newNum = out.readInt32BE(0)
assert(nums[0] === newNum)
out.writeInt32BE(nums[1], 4)
newNum = out.readInt32BE(4)
assert(nums[1] === newNum)
out.writeInt32BE(nums[2], 8)
newNum = out.readInt32BE(8)
assert(nums[2] === newNum)



// ################################### WRITE INFINITY TESTS #####################################
// write/read Infinity as a float
var buf = new Buffer(4)
assert(buf.writeFloatBE(Infinity, 0) === 4)
assert(buf.readFloatBE(0) === Infinity)

// write/read -Infinity as a float
var buf = new Buffer(4)
assert(buf.writeFloatBE(-Infinity, 0) === 4)
assert(buf.readFloatBE(0) === -Infinity)

// write/read Infinity as a double
var buf = new Buffer(8)
assert(buf.writeDoubleBE(Infinity, 0) === 8)
assert(buf.readDoubleBE(0) === Infinity)

// write/read -Infinity as a double
var buf = new Buffer(8)
assert(buf.writeDoubleBE(-Infinity, 0) === 8)
assert(buf.readDoubleBE(0) === -Infinity)

// write/read float greater than max
var buf = new Buffer(4)
assert(buf.writeFloatBE(4e38, 0) === 4)
assert(buf.readFloatBE(0) === Infinity)

// write/read float less than min
var buf = new Buffer(4)
assert(buf.writeFloatBE(-4e40, 0) === 4)
assert(buf.readFloatBE(0) === -Infinity)


alert('nodebuffer-tests.js:\n\nAutomatic tests completed.\n\nThere are some manual tests that require uncommenting lines to confirm that the code throws errors when it should.')
